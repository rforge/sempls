...
# for 'sempls' objects
pathDiagram <- function(sempls, ...){
    UseMethod("pathDiagram")
    }

pathDiagram.sempls <- function(sempls, file, min.rank=NULL, max.rank=NULL, same.rank=NULL,
    ...
    latent <- sempls$model$latent           
    variables <- c(sempls$model$manifest,latent)
    parameters <- rownames(sempls$coefficients)
    ...
    cat(file = handle, paste("digraph \"", deparse(substitute(sempls)), 
        "\" {\n", sep = ""))
    ...
    values <- round(sempls$coefficients$Estimate, digits)
    labels <- if (edge.labels == "names") 
        parameters
    else if (edge.labels == "values") 
        values
    else paste(parameters, values, sep = "=")
    path <- sub(' -> ', '" -> "', sempls$coefficients[,1])
    cat(file=handle,
        if(full){
          paste(' "', path, '" [label="', labels, '"];\n', sep="")
        }
        else{paste(' "', path[-(1:length(sempls$model$manifest))],
                   '" [label="', labels[-(1:length(sempls$model$manifest))], '"];\n',
                   sep="")}
       )
    cat(file = handle, "}\n")
